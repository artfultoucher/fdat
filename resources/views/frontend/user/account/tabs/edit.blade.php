{{ html()->modelForm($logged_in_user, 'PATCH', route('frontend.user.profile.update'))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
    ...
    @hasrole('student')
        <div class="row">
            <div class="col">
                <div class="form-group">
                    {{ html()->label('UCC Student ID')->for('studentid') }}
                    {{ html()->text('studentid')
                        ->class('form-control')
    ...
    <div class="row">
        <div class="col">
           <div class="form-group">Subscribed matters. See <a href="{{route('frontend.matters')}}">here for details</a>.
             <div class="form-control">
              @foreach ($logged_in_user->matters() as $matter)
                 <div class="form-check form-check-inline">
                  <label class="form-check-label">
                  @if ($logged_in_user->has_subscribed($matter))
                    <input class="form-check-input" checked value="{{$matter}}" name="subscr[]" data-toggle="toggle" type="checkbox"> {{$matter}}
                  @else
                    <input class="form-check-input" value="{{$matter}}" name="subscr[]" data-toggle="toggle" type="checkbox"> {{$matter}}
                  @endif
                </label>
              </div>
              @endforeach
